                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('contract_templates.update', $contractTemplate->id) }}" method="POST">
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" name="name" id="name" class="form-control" value="{{ old('name', $contractTemplate->name) }}" required>
                        </div>

                        <div class="form-group mb-4">
                            <label for="template">{{ __("Contract Template") }}</label>
                            <textarea name="template" id="template" class="form-control" rows="10" required>{{ old('template', $contractTemplate->template) }}</textarea>
                        </div>

                        <div class="d-flex gap-3 mt-5">
                            <button type="submit" class="btn btn-success">{{ __("Update Contract Template") }}</button>
